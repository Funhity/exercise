...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
	...
	Class.prototype.pullData = function(curr, loadIndex) {
		...
		if(options.url) { //Ajax请求
			...
			for(var whereName in options.where) {
				var whereVal = options.where[whereName];
				if(whereVal === undefined || $.trim(whereVal) === '') {
					delete options.where[whereName];
				}
			}
			/*----------------------新增代码-------------------------*/
			$.ajax({
			...
			render = function() { //后续性能提升的重点
				...
				layui.each(data, function(i1, item1) {
					...
					that.eachCols(function(i3, item3) {
						...
						/*----------------------新增代码-------------------------*/
						//处理0 1 代表是否 加个判断防止把id唯一的变成是
						if(item3.datetimeformat) { //加入 datatimeformat:true 格式化日期时间
							if(content != '') {
                                content = new Date(content).format("yyyy-MM-dd hh:mm:ss");
							}
						}
						if(item3.dateformat) { //加入 datatimeformat:true 格式化日期
                            if(content != '') {
                                content = new Date(content).format("yyyy-MM-dd");
                            }
						}
						if(item3.selected) {
							$.each(eval(item3.jsontext), function(key, value) {
								content = content == value.value ? value.text : content
								if(content == value.value) content = value.text
							})
						}
						/*----------------------新增代码-------------------------*/
						...
						//td内容
						/*----------------------新增代码-------------------------*/
						var td = ['<td  data-jsontext="' + (item3.jsontext || i3) + '" data-field="' + (item3.field || i3) + '"' + function() {
							/*----------------------新增代码-------------------------*/
							//var td = ['<td data-field="'+ field +'" '+ function(){
							var attr = [];
							/*----------------------新增代码-------------------------*/
							if(item3.selected) attr.push(' data-select="true"'); //是否允许单元格编辑
							if(item3.disabled) attr.push(' data-disable="true"'); //是否允许单元格编辑
							/*----------------------新增代码-------------------------*/
							...
							/*----------------------新增代码-------------------------*/
							if(item3.isCheckbox) return 'tdCheck laytable-cell-' + options.index + '-' + (item3.field || i3); //间距
							/*----------------------新增代码-------------------------*/
							...
							/*----------------------新增代码-------------------------*/
							if(item3.isCheckbox && item3.checkConfig) {
								var checkConfig = "";
								if(content == item3.checkConfig.checked) {
									checkConfig = 'data-check-config=' + JSON.stringify(item3.checkConfig)
									return '<input type="checkbox" class="layui-table-check" name="' + item3.checkConfig.name + '" lay-skin="primary" ' + checkConfig + ' checked>'
								} else {
									return '<input type="checkbox" class="layui-table-check" name="' + item3.checkConfig.name + '" lay-skin="primary" ' + checkConfig + ' >'
								}
							}
							/*----------------------新增代码-------------------------*/
	...
	Class.prototype.fullSize = function() {
		var that = this,
			...
			bodyHeight;
		/*----------新增代码---页面高度计算错误bug----------*/
		that.fullHeightGap = that.fullHeightGap || 290;
        /*----------新增代码----------*/
	...
	Class.prototype.scrollPatch = function() {
		...
		if(that.autoColNums && outWidth < 5 && !that.scrollPatchWStatus) {
			...
			that.getCssRule(field, function(item) {
				...
				item.style.width = (parseFloat(width) - getScrollWidth - that.layMain.width()) + 'px';//修改代码
				//二次校验，如果仍然出现横向滚动条
	...
	Class.prototype.events = function() {
		...
		/*----------------------新增代码-------------------------*/
		that.layBody.on('click', ".laytable-cell-tdCheck", function() {

			var othis = $(this),
				field = othis.parent().data('field'),
				index = othis.parents('tr').eq(0).data('index'),
				data = table.cache[that.key][index],
				checkConfig = othis.find("input[type=checkbox]").data("checkConfig"),
				value = othis.find(".layui-form-checkbox").hasClass("layui-form-checked") ? checkConfig.checked : checkConfig.unchecked
		...
		that.layBody.on('change', '.' + ELEM_EDIT, function() {
			var othis = $(this),
				...
				field = othis.parent().data('field'),
			/*----------------------新增代码-------------------------*/
			jsontext = othis.parent().data('jsontext'),
				/*----------------------新增代码-------------------------*/
		...
		/*----------------------新增代码-------------------------*/
		that.layBody.on('change', '.layui-table-select', function() {
			// var othis = $(this)
			//     ,value = this.value
			//     ,field = othis.parent().data('field')
			//     ,jsontext = othis.parent().data('jsontext')
			...
			//     ,field: field
			// });
		}).on('blur', '.layui-table-select', function() {
			$(this).prev().show()
			var templet, othis = $(this),
				field = othis.parent().data('field'),
				index = othis.parents('tr').eq(0).data('index'),
			...
			data[field] = value; //更新缓存中的值
			if(!this.options) {
				var textsel = this.options[index].text
			}
			layui.event.call(this, MOD_NAME, 'edit(' + filter + ')', {
				value: value,
				data: data,
			...
			$.each(eval(jsontext), function(i, item) {
				if(value == item.value) value = item.text
			})
			othis.parent().data('content', value);
			$(othis).prev().html(value)
			othis.remove();
		});
		/*----------------------新增代码-------------------------*/
		//单元格事件
		that.layBody.on('click', 'td', function() {
			...
			/*----------------------新增代码-------------------------*/
			//下拉菜单的填充方法
			function createSel(data) {
				var sel = '<select class="form-control layui-table-select">'
				layui.each(eval(data), function(i, v) {
					sel += '<option value="' + v.value + '" data-id="' + v.id + '">' + v.text + '</option>'
				})
				sel += '</select>'
				return sel;
			}
			//显示下拉菜单
			if(othis.data('select')) {
				var beforeVal = othis.text();
				othis.find(".layui-table-cell").hide()
				var selhtml = $(createSel(othis.data('jsontext')));
				selhtml.find("option:contains('" + beforeVal + "')").attr("selected", true);
				othis.find('.form-control')[0] || othis.append(selhtml);
				othis.find('.form-control').show()
				return selhtml.focus();
			}
			/*----------------------新增代码-------------------------*/
			//如果出现省略，则可查看更多
