package com.huaxia.rms.query.util;

import com.huaxia.rms.pojo.PageInfo;
import com.huaxia.rms.query.entity.Column;
import com.huaxia.rms.query.entity.Query;
import com.huaxia.rms.query.entity.QueryCondition;
import com.huaxia.rms.query.filter.ConditionOperator;
import com.huaxia.rms.query.filter.IFilter;
import com.huaxia.rms.query.filter.StringFilter;
import com.huaxia.rms.query.pojo.QueryDefinition;
import com.huaxia.rms.utils.DateUtil;
import com.huaxia.rms.utils.JSonHelper;
...
public class QueryUtil {
     ...
     * @return 查询参数
     * @throws Exception xml配置不存在 queryId为空异常
     */
    public static Query getQuery(QueryCondition queryCondition) throws Exception {
        ...
        if (!StrUtil.isEmpty(queryId)) {
            ...
            if (query == null) {
                throw new Exception("queryId为【" + queryId + "】的xml配置不存在");
            } else {
                return query;
            }
        } else {
            throw new Exception("queryId为空，请指定queryId!");
        }
    }
...
//     * @param pageInfo       分页信息
//     * @return 数据列表
//     * @throws Exception service和method不存在异常，以及method内部异常
//     */
//    public static List queryByService(QueryCondition queryCondition, Query query, PageInfo pageInfo) throws Exception {
//        Object service = springContextUtil.getBean(query.getService());
//        if (service == null) {
//            throw new Exception("service为【" + query.getService() + "】的接口不存在");
//        }
//        Class clazz = service.getClass();
//        if (StrUtil.isEmpty(query.getMethod())) {
//            throw new Exception("method为【" + query.getMethod() + "】不存在");
//        }
//        try {
...
//        } catch (Exception ex) {
//            throw new Exception("【" + query.getService() + "】接口的【" + query.getMethod() + "】方法调用异常");
//        }
//    }
...
//     */
//    public static Map getSqlParams(QueryCondition queryCondition, Query query) throws Exception{
...
//    }
     ...
     * @return map 处理后的值 operator返回操作符（日期操作 EQ 改为BETWEEN） value
     */
    private static Map<String, Object> getValueForOperator(ConditionOperator operator, Object value, Class clazz) throws Exception {
        Object newValue;
        try {
            newValue = getValueByOperator(operator, value, clazz);
        } catch (Exception ex) {
            throw new Exception("JSonHelper.getValue发生异常,转化的参数为：" + value.toString());
        }
        if (newValue == null)
            return null;
        //对日期格式的处理
        if (Date.class.equals(clazz) || Date.class.equals(clazz.getSuperclass())) {
            try {
                ...
                throw new Exception("DateUtil.getLastMilliSecond日期转化发生异常");
            }
        }
        Map<String, Object> map = new HashMap<>();
        map.put("operator", operator);
     ...
     * @return 查询条件拼接对象
     */
    public static IFilter generateFilter(QueryCondition condition, Query query, StringBuilder sqlBuilder) throws Exception {
        ...
        for (Map<String, Object> map : conditions) {
            ...
            try {
                ...
                throw new Exception("转化类【" + column.getClassType() + "】出现异常！");
            }
            //操
            // 作需要值，并且有值
...
//     * @return 离线查询criteria
//     */
//    public static DetachedCriteria generateCriteria(QueryCondition condition, Query query, Class entityClazz) throws Exception {
...
//                    throw new Exception("排序" + str + "的配置有问题,可能未配置asc或desc，请检查");
//                }
