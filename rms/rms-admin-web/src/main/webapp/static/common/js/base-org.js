...
(function ($, window, document, undefined) {
    ...
    BaseOrg.prototype.selectOrg = function () {
        ...
        if (selectedNode.length > 0) {
            ...
            //填充机构代码及管理员人员信息
            if(this.options.isFill == true) {
                ajaxPost(basePath + "/org/get/" + selectedNode[0].code +".do", null, function (data) {
                    //self.form.initFormData(data);
                    $("#username").val(data.user.name);
                    $("#usercode").val(data.user.code);
                    $("#usermobile").val(data.user.mobile);

                    $("#code").val(data.code);
                    $("#shortName").val(data.shortName);
                    $("#orgCategoryCodes").val(data.orgCategoryCodes);
                    $("#orgCategoryNames").val(data.orgCategoryNames);
                    $("#maxuser").val(data.maxuser == -1 ? "": data.maxuser);
