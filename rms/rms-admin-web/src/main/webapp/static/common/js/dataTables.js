...
//'use strict';
    ...
    CommonTable.prototype.initTable = function (tableId, queryId, searchDiv) {
        /**
         * 分页，前台可显示的表字段名及字段值对应的数据
         * 前台可显示的表字段是配置在query/base.xml中的
         */
        ...
        for (var i = 0; i < this.data.query.columnList.length; i++) {
            ...
            //obj["operator"] = "text-" + column.operator;
        ...
        this.table = $('#' + tableId).DataTable($.extend({
            ...
            "language": { // 中文支持
                "sUrl": basePath + "/static/common/json/zh_CN.json"
            },
            "displayLength": that.data.pageInfo.pageSize,// 每页记录条数，默认为10
            "serverSide": true,
            "ajaxDataProp": "data",
            "ajaxSource": basePath + "/user/loadData.do",
...
//刷新当前页面，并定位到行
    CommonTable.prototype.reloadRowData = function (rowId) {
        ...
        //console.log("-----this.table.page: " + this.table.page(1).draw(false));
        //console.log("-----this.table.page: " + this.table.page(pageIndex));
...
// isCondition默认为true;likeOption默认false 即不拼接%
    CommonTable.prototype.fnGetConditions = function (searchDiv) {
        ...
        if (searchDiv !== null && searchDiv.length > 0) {
            ...
            // var sele = searchDiv.find('select');
            // console.log("sele: "+sele);
            // console.log("sele.val(): "+sele.val());
            // if(sele && sele.val()) {
            //     conditions.push(sele.attr("name"),sele.val());
            // }
        } else {
    ...
    CommonTable.prototype.getServerData = function (pageInfo, tableId) {
        ...
        var reqParam = {
            ...
            conditions: this.fnGetConditions(this.searchDiv)//查询条件的参数值
        };
        ...
        ajaxPost(basePath + "/query/loadData.do", {"reqObj": this.toJSONString(reqParam)}, function (result, status) {
            retData = result;
        });
        var start = 0;
