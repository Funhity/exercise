...
<script>
    ...
    var orgCtrl = {
        ...
        move: function (selectedNode) {
            ...
            if (selectedNode.nodes) {
                modals.info('该节点含有子节点，请先迁移子节点');
                return false;
            }
            modals.confirm('是否迁移该节点', function () {
                ajaxPost(basePath + "/org/move/" + selectedNode.code+".do", null, function (data) {
                    if (data.success) {
                        modals.correct('迁移成功');
                    } else {
                        modals.info(data.message);
...
//                var code = nodes ? nodes[nodes.length - 1].code : null;
                ...
                //alert(" parentNode.code:" +  parentNode.code);
                //var layer = "0";
                if (parentNode) {
